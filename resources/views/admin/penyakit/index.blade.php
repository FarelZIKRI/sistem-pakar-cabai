...
@section('content')
    <div class="bg-white shadow-lg rounded-lg overflow-hidden">
        <div class="p-6 bg-white border-b border-gray-200 flex justify-between items-center">
            <h2 class="text-xl font-bold text-gray-800">Daftar Penyakit</h2>
                ...
                <tbody>
                    @foreach($penyakits as $p)
                        <tr class="hover:bg-gray-100">
                            <td class="py-4 px-6 border-b border-gray-200">{{ $p->kode }}</td>
                            <td class="py-4 px-6 border-b border-gray-200">{{ $p->nama }}</td>
                            <td class="py-4 px-6 border-b border-gray-200">
                                {{ \Illuminate\Support\Str::limit($p->deskripsi, 100) }}</td>
                            <td class="py-4 px-6 border-b border-gray-200">
                                <a href="{{ route('admin.penyakit.edit', $p->id) }}"
                                    class="text-blue-500 hover:text-blue-700 mr-4"><i class="fas fa-edit"></i></a>
                                <form action="{{ route('admin.penyakit.destroy', $p->id) }}" method="POST" class="inline"
